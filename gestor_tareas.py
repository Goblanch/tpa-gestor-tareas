...
class GestorTareas:
    ...
    def agregar_tarea(self, tarea: Tarea) -> None:
        if not isinstance(tarea, Tarea):
            raise TypeError("    -> [ERROR] [GESTOR] Solo se pueden agregar objetos de tipo tarea.")

        if self.buscar_por_nombre(tarea.titulo) is not None:
            raise ValueError(f"    -> [WRN] [GESTOR] Ya existe la tarea {tarea.titulo}. No se añade.")
    ...
    def eliminar_tarea(self, titulo: str) -> Tarea:
        ...
        if tarea is None:
            raise ValueError(f"    -> [ERROR] [GESTOR] No existe ninguna tarea con el título {titulo}")
    ...
    def mostrar_tarea(self, titulo: str) -> None:
        tarea = self.buscar_por_nombre(titulo)
        if tarea is None:
            print(f"    -> [WRN] [GESTOR] No se encontró ninguna tarea con el título {titulo}")
            return
    ...
    def mostrar_lista_tareas(self):
        if not self._tareas:
            print("    -> [WRN] [GESTOR] No hay tareas registradas")
            return
